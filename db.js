...
function searchAccounts(q, limit=50) {
  const term = `%${q}%`;
  return db.prepare(
    "SELECT user_id,username,balance FROM accounts WHERE username LIKE ? OR user_id LIKE ? ORDER BY username ASC LIMIT ?"
  ).all(term, term, limit);
}

module.exports = { getOrCreateAccount, deposit, withdraw, transfer, applyDelta, resetBalance, leaderboard, searchAccounts };
